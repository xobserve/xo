...
If we want query data from http api, you should transform the query result to above format to make it work.


## Panel data transform

Nomatter how the data is transformed, it must be compatible with the `final data format` required by the chart, otherwise the chart will not be displayed correctly.
...
The final data format is very important in Datav, no matter you want to transform the existing data or convert the incompatible data from external query, the ultimate goal is to achieve the final data format.

If you want to know the final data format of a chart type, it's also very simple: use TestData as datasource and select the chart type you need, then open `Panel Debug -> Panel Data`, the data format you saw is the final data format.
