...
我们将创建一个变量，用于从 jaeger http api 查询数据：`http://localhost:16686/api/services/frontend/operations`。
...
大家应该有注意到，在 `Query values` 字段中，我们使用了 `service` 变量：`http://localhost:16686/api/services/${service}/operations`，这意味着**我们可以在 HTTP 请求 url 中使用变量**。
