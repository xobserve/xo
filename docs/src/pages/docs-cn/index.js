  ...
  const latestUpdates = [
    {
      ...
      description:
        isChinese ? '今天我们很激动，在经过半年的日夜努力后，Datav终于发布了第一个正式版本，当前的功能基于Grafana V7.2，同时增加了很多新的特性' : "Today we're excitedto release the first version of Datav, current version is built based on grafana v7.2 with some new features",
    }
  ]
  ...
  return (
    <div className="px-4 sm:px-6 xl:px-8 pt-10 pb-16">
      <h1 className="text-5xl leading-none font-extrabold text-gray-900 tracking-tight mb-4">
        {isChinese ? '开始了解Datav' : 'Getting started with Datav'}
      </h1>
      <p className="text-2xl tracking-tight mb-10">
        {isChinese ? '使用我们推荐的方式来学习' : "Learn Datav the way that best matches your learning style."}
      </p>
          ...
          <div className="w-full relative text-white overflow-hidden rounded-3xl flex shadow-lg">
            <div className={`w-full flex md:flex-col bg-gradient-to-br ${gradients.violet[0]}`}>
              <div className="sm:max-w-sm sm:flex-none md:w-auto md:flex-auto flex flex-col items-start relative z-10 p-6 xl:p-8">
                <h2 className="text-xl font-semibold mb-2 text-shadow">{isChinese ? '阅读文档' : 'Read the docs'}</h2>
        ...
        <section className="flex">
          <div className="w-full relative text-white overflow-hidden rounded-3xl flex shadow-lg">
            <div className={`w-full flex md:flex-col bg-gradient-to-br ${gradients.pink[0]}`}>
              <div className="sm:max-w-sm sm:flex-none md:w-auto md:flex-auto flex flex-col items-start relative z-10 p-6 xl:p-8">
                <h2 className="text-xl font-semibold mb-2 text-shadow">  {isChinese ? '在线Demo' : 'Try it in the browser'}</h2>
                <p className="font-medium text-rose-100 text-shadow mb-4">
                  {isChinese ? '使用我们提供的在线Demo来了解Datav' : 'Build something with Tailwind in our online playground.'}
                </p>
                <a
      ...
      <section>
        <header className="flex items-center justify-between mt-16 mb-8">
          <h2 className="text-3xl tracking-tight font-extrabold text-gray-900">{isChinese ? '最近更新' :"Latest Updates"}</h2>
