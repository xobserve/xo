...
package xobservespanmetricsprocessor
...
type processorImp struct {
	...
	dimensions             []dimension // xobserve_latency metric
	callDimensions         []dimension // xobserve_calls_total metric
	dbCallDimensions       []dimension // xobserve_db_latency_* metric
	externalCallDimensions []dimension // xobserve_external_call_latency_* metric

	// The starting time of the data points.
	startTimestamp pcommon.Timestamp

	// Histogram.
	histograms    map[metricKey]*histogramData // xobserve_latency metric
	latencyBounds []float64

	callHistograms    map[metricKey]*histogramData // xobserve_calls_total metric
	callLatencyBounds []float64

	dbCallHistograms    map[metricKey]*histogramData // xobserve_db_latency_* metric
	dbCallLatencyBounds []float64

	externalCallHistograms    map[metricKey]*histogramData // xobserve_external_call_latency_* metric
...
func newProcessor(logger *zap.Logger, instanceID string, config component.Config, nextConsumer consumer.Traces) (*processorImp, error) {
	logger.Info("Building xobservespanmetricsprocessor")
...
func (p *processorImp) Start(ctx context.Context, host component.Host) error {
	p.logger.Info("Starting xobservespanmetricsprocessor with config", zap.Any("config", p.config))
	...
	for k, exp := range exporters[component.DataTypeMetrics] {
		...
		p.logger.Debug("Looking for xobservespanmetrics exporter from available exporters",
			zap.String("xobservespanmetrics-exporter", p.config.MetricsExporter),
			zap.Any("available-exporters", availableMetricsExporters),
		)
		if k.String() == p.config.MetricsExporter {
			p.metricsExporter = metricsExp
			p.logger.Info("Found exporter", zap.String("xobservespanmetrics-exporter", p.config.MetricsExporter))
			break
		}
	}
	...
	p.logger.Info("Started xobservespanmetricsprocessor")
	return nil
}

// Shutdown implements the component.Component interface.
func (p *processorImp) Shutdown(ctx context.Context) error {
	p.logger.Info("Shutting down xobservespanmetricsprocessor")
	return nil
}
...
func (p *processorImp) buildMetrics() (pmetric.Metrics, error) {
	...
	ilm.Scope().SetName("xobservespanmetricsprocessor")
...
func (p *processorImp) collectLatencyMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mLatency.SetName("xobserve_latency")
	mLatency.SetUnit("ms")
...
func (p *processorImp) collectDBCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mDBCallSum.SetName("xobserve_db_latency_sum")
	...
	mDBCallCount.SetName("xobserve_db_latency_count")
...
func (p *processorImp) collectExternalCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mExternalCallSum.SetName("xobserve_external_call_latency_sum")
	...
	mExternalCallCount.SetName("xobserve_external_call_latency_count")
...
func (p *processorImp) collectCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mCalls.SetName("xobserve_calls_total")
