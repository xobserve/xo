...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions int) bool {
	...
	assert.Equal(t, "xobservespanmetricsprocessor", ilm.At(0).Scope().Name())

	m := ilm.At(0).Metrics()
	// 6 metrics: xobserve_calls_total, xobserve_latency, xobserve_db_latency_sum
	// xobserve_db_latency_count, xobserve_external_latency_sum, xobserve_external_latency_count
	require.Equal(t, 6, m.Len())

	seenMetricIDs := make(map[metricID]bool)
	// The first 3 data points are for call counts.
	assert.Equal(t, "xobserve_calls_total", m.At(0).Name())
	...
	assert.Equal(t, "xobserve_latency", m.At(1).Name())
