...
import (
	...
	"github.com/DataObserve/observex/otel-collector/processor/observexspanmetricsprocessor/mocks"
)
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions int) bool {
	...
	assert.Equal(t, "observexspanmetricsprocessor", ilm.At(0).Scope().Name())

	m := ilm.At(0).Metrics()
	// 6 metrics: observex_calls_total, observex_latency, observex_db_latency_sum
	// observex_db_latency_count, observex_external_latency_sum, observex_external_latency_count
	require.Equal(t, 6, m.Len())

	seenMetricIDs := make(map[metricID]bool)
	// The first 3 data points are for call counts.
	assert.Equal(t, "observex_calls_total", m.At(0).Name())
	...
	assert.Equal(t, "observex_latency", m.At(1).Name())
