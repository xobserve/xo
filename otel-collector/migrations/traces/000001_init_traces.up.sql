CREATE TABLE xobserve_traces.trace_index
(
...
CREATE TABLE IF NOT EXISTS xobserve_traces.distributed_trace_index ON CLUSTER cluster AS xobserve_traces.trace_index
ENGINE = Distributed("cluster", "xobserve_traces", trace_index, cityHash64(traceId));


CREATE TABLE xobserve_traces.trace_spans
(
    `startTime` UInt64 CODEC(DoubleDelta, LZ4),
    `traceId` FixedString(32) CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS xobserve_traces.distributed_trace_spans ON CLUSTER cluster AS xobserve_traces.trace_spans
ENGINE = Distributed("cluster", "xobserve_traces", trace_spans, cityHash64(traceId));
...
ENGINE = Distributed("cluster", "xobserve_traces", top_level_operations, cityHash64(rand()));
...
ENGINE = Distributed("cluster", "xobserve_traces", service_operations, cityHash64(rand()));
...
CREATE TABLE IF NOT EXISTS xobserve_traces.distributed_usage_explorer ON CLUSTER cluster AS xobserve_traces.usage_explorer
ENGINE = Distributed("cluster", "xobserve_traces", usage_explorer, cityHash64(rand()));

CREATE MATERIALIZED VIEW IF NOT EXISTS xobserve_traces.usage_explorer_mv ON CLUSTER cluster
...
CREATE TABLE IF NOT EXISTS xobserve_traces.distributed_trace_error_index ON CLUSTER cluster AS xobserve_traces.trace_error_index
ENGINE = Distributed("cluster", "xobserve_traces", trace_error_index, cityHash64(groupID));
...
ENGINE = Distributed("cluster", "xobserve_traces", span_attributes_keys, cityHash64(rand()));
...
CREATE TABLE IF NOT EXISTS xobserve_traces.distributed_span_attributes ON CLUSTER cluster AS xobserve_traces.span_attributes
ENGINE = Distributed("cluster", "xobserve_traces", span_attributes, cityHash64(rand()));