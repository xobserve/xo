...
import { GraphThresholdsStyleConfig, GraphTresholdsStyleMode } from '../types';
...
export function getThresholdsDrawHook(options: UPlotThresholdOptions) {
  ...
  function addLines(u: uPlot, yScaleKey: string, steps: Threshold[], theme: any) {
    let ctx = u.ctx;
  ...
  function addAreas(u: uPlot, yScaleKey: string, steps: Threshold[], theme: any) {
    let ctx = u.ctx;
