...
export default function GraphTooltip(this: any, elem: JQuery, dashboard: DashboardModel, panel: GraphPanelCtrl, getSeriesFn: any) {
  const self = this;
  ...
  this.getMultiSeriesPlotHoverInfo = function(seriesList: any[], pos: { x: number }) {
    ...
    for (i = 0; i < seriesList.length; i++) {
      ...
      if (!series.data.length || (panel.graphOptions.legend.hideEmpty && series.allIsNull)) {
        // Init value so that it does not brake series sorting
        results[0].push({ hidden: true, value: 0 });
        continue;
      }

      if (!series.data.length || (panel.graphOptions.legend.hideZero && series.allIsZero)) {
        // Init value so that it does not brake series sorting
        results[0].push({ hidden: true, value: 0 });
        continue;
      }
      ...
      if (series.stack) {
        if (panel.graphOptions.tooltip.value_type === 'individual') {
  ...
  elem.mouseleave(() => {
    if (panel.graphOptions.tooltip?.shared) {
  ...
  this.show = (pos: any, item: any) => {
    ...
    let allSeriesMode = panel.graphOptions.tooltip.shared;
    ...
    if (allSeriesMode) {
      ...
      // option is enabled.
      if (panel.graphOptions.tooltip.sort === 2) {
        seriesHoverInfo.sort((a: { value: number }, b: { value: number }) => {
          return b.value - a.value;
        });
      } else if (panel.graphOptions.tooltip.sort === 1) {
        seriesHoverInfo.sort((a: { value: number }, b: { value: number }) => {
      ...
      for (i = 0; i < seriesHoverInfo.length; i++) {
        ...
        seriesHtml +=
          ...
          seriesHtml +=    `
          <span role="img" aria-label="minus" class="anticon anticon-minus" style="font-size:13px;font-weight:bold;color: ` + color +`;">
            <svg viewBox="64 64 896 896" focusable="false" class="" data-icon="minus" width="1em" height="1em" fill="currentColor" aria-hidden="true">
              <path d="M872 474H188c-4.4 0-8 3.6-8 8v60c0 4.4 3.6 8 8 8h720c4.4 0 8-3.6 8-8v-60c0-4.4-3.6-8-8-8z"></path>
            </svg>
          </span>`  + label + ':</div>'
      ...
      if (panel.graphOptions.stack && panel.graphOptions.tooltip.value_type === 'individual') {
