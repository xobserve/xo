...
export class ThresholdManager {
  ...
  constructor(private panelCtrl: GraphPanelCtrl) {}
  ...
  prepare(elem: JQuery, data:  TimeSeries[]) {
    ...
    if (this.shouldDrawHandles()) {
      const thresholdMargin = this.panelCtrl.graphOptions.thresholds.length > 1 ? '220px' : '110px';
  ...
  draw(plot: any) {
    this.thresholds = this.panelCtrl.graphOptions.thresholds;
    this.plot = plot;
  ...
  addFlotOptions(options: any, panel: GraphPanelCtrl) {
    ...
    for (i = 0; i < panel.graphOptions.thresholds.length; i++) {
      threshold = panel.graphOptions.thresholds[i];
      ...
      switch (threshold.op) {
        case 'gt': {
          ...
          if (panel.graphOptions.thresholds.length > i + 1) {
            other = panel.graphOptions.thresholds[i + 1];
