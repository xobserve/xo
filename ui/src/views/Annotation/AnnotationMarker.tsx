...
import { Box, Flex, HStack, Popover, PopoverArrow, PopoverBody, PopoverContent, PopoverTrigger, Portal, Text, VStack, useColorMode } from "@chakra-ui/react";
                ...
                <Box cursor="pointer" className="annotation-marker">
                    {
                        !isRegionAnnotation ? <div
                            style={{
                                ...
                                borderBottom: `4px solid ${paletteColorNameToHex(annotation.color,colorMode)}`
                            }}
                        /> : <div
                            style={{
                                width: `${width}px`,
                ...
                <PopoverContent minWidth="350px" p="0">
                    ...
                    <PopoverBody p="0">
                        <Flex justifyContent="space-between" alignItems="center" fontSize="0.8rem" className="bordered-bottom" px="1">
                            {!isEmpty(annotation.namespace) ? <HStack spacing={3}>
                            ...
                            </HStack> : <HStack spacing={2} p="2">
                                <Text minWidth="fit-content">{dateTimeFormat(annotation.time * 1000)}</Text>
                                <Text>-</Text>
                                <Text minWidth="fit-content">{dateTimeFormat((annotation.time + durationToSeconds(annotation.duration)) * 1000)}</Text>
                            </HStack>}
                            {!isEmpty(annotation.namespace) && <HStack spacing={3} className="action-icon">
