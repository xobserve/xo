...
import { StoreState, ThunkDispatch, ThunkResult } from 'src/types';
import {
  QueryVariableModel,
  VariableOption,
  VariableRefresh,
...
import { containsSearchFilter } from 'src/core/library/utils/templating';
...
const fetchTagValues = (tagText: string): ThunkResult<Promise<string[]>> => {
  return async (dispatch, getState) => {
    ...
    const datasource = await getDataSourceService().get(variable.datasource ?? '');
