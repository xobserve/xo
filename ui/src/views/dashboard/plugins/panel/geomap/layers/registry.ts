...
const hasAlphaPanels = false
const defaultBaseLayerConfig = undefined
...
export const defaultBaseLayer: MapLayerRegistryItem = {
  ...
  create: (map: OpenLayersMap, options: MapLayerOptions, theme: any) => {
    const serverLayerType = defaultBaseLayerConfig?.type;
    if (serverLayerType) {
      const layer = geomapLayerRegistry.getIfExists(serverLayerType);
      if (!layer) {
        throw new Error('Invalid basemap configuration on server');
      }
      return layer.create(map, defaultBaseLayerConfig!, theme);
    }
