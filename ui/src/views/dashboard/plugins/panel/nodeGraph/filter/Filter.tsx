                ...
                <DrawerContent minW="300px">
                    ...
                    <DrawerBody px="1">
                        {tempRule && <><Wrap fontSize="0.8rem" spacing="1" mt="1">
                            ...
                            {
                                tempRule.operator == FilterOperator.Regex ? <Input variant='flushed' value={tempRule.value} size="xs" width="fit-content" placeholder={placeholder} onChange={e => {
                                    const v = e.currentTarget.value;
                        ...
                        <Alert status='success' position="absolute" bottom="0" left="0" fontSize="0.9rem">
                            <VStack alignItems="left" spacing="1">
                                ...
                                <Text>1. <strong>from top to bottom</strong>  e.g (((a AND b) OR c) AND d)</Text>
                                <Text>2. <strong>from nodes to edges</strong>,  edges rules will be applied only if all of the nodes rules already be applied </Text>
                                <Text>3. <strong>edges rules are applied on the result of nodes rules</strong>, e.g if the nodes rules caused Node A to be invisible, as a result it's edges A1 and A2 are also invisible, then the edges rules will be applied, but they will have no effects on the visible of A1, A2</Text>
                                <Text> <strong>Rules will be stored in your browser</strong>, so you will see them every time when opening this dashboard.</Text>
                            </VStack>
                        </Alert>
                    </DrawerBody>
                </DrawerContent>
