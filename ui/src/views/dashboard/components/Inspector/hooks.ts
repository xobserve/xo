import { DataQueryError, DataSourceApi, PanelData, PanelPlugin } from 'src/packages/datav-core/src';
    ...
    if (data && data.series && targets.length) {
      for (const frame of data.series) {
        if (frame.meta && frame.meta.custom) {
          ...
          const dataSource = await getDataSourceService().get(targets[0].datasource);
