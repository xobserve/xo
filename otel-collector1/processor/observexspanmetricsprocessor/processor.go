...
import (
	...
	"github.com/DataObserve/observex/otel-collector/processor/observexspanmetricsprocessor/internal/cache"
)
...
type processorImp struct {
	...
	dimensions             []dimension // observex_latency metric
	callDimensions         []dimension // observex_calls_total metric
	dbCallDimensions       []dimension // observex_db_latency_* metric
	externalCallDimensions []dimension // observex_external_call_latency_* metric

	// The starting time of the data points.
	startTimestamp pcommon.Timestamp

	// Histogram.
	histograms    map[metricKey]*histogramData // observex_latency metric
	latencyBounds []float64

	callHistograms    map[metricKey]*histogramData // observex_calls_total metric
	callLatencyBounds []float64

	dbCallHistograms    map[metricKey]*histogramData // observex_db_latency_* metric
	dbCallLatencyBounds []float64

	externalCallHistograms    map[metricKey]*histogramData // observex_external_call_latency_* metric
...
func newProcessor(logger *zap.Logger, instanceID string, config component.Config, nextConsumer consumer.Traces) (*processorImp, error) {
	logger.Info("Building observexspanmetricsprocessor")
...
func (p *processorImp) Start(ctx context.Context, host component.Host) error {
	p.logger.Info("Starting observexspanmetricsprocessor with config", zap.Any("config", p.config))
	...
	for k, exp := range exporters[component.DataTypeMetrics] {
		...
		p.logger.Debug("Looking for observexspanmetrics exporter from available exporters",
			zap.String("observexspanmetrics-exporter", p.config.MetricsExporter),
			zap.Any("available-exporters", availableMetricsExporters),
		)
		if k.String() == p.config.MetricsExporter {
			p.metricsExporter = metricsExp
			p.logger.Info("Found exporter", zap.String("observexspanmetrics-exporter", p.config.MetricsExporter))
			break
		}
	}
	...
	p.logger.Info("Started observexspanmetricsprocessor")
	return nil
}

// Shutdown implements the component.Component interface.
func (p *processorImp) Shutdown(ctx context.Context) error {
	p.logger.Info("Shutting down observexspanmetricsprocessor")
	return nil
}
...
func (p *processorImp) buildMetrics() (pmetric.Metrics, error) {
	...
	ilm.Scope().SetName("observexspanmetricsprocessor")
...
func (p *processorImp) collectLatencyMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mLatency.SetName("observex_latency")
	mLatency.SetUnit("ms")
...
func (p *processorImp) collectDBCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mDBCallSum.SetName("observex_db_latency_sum")
	...
	mDBCallCount.SetName("observex_db_latency_count")
...
func (p *processorImp) collectExternalCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mExternalCallSum.SetName("observex_external_call_latency_sum")
	...
	mExternalCallCount.SetName("observex_external_call_latency_count")
...
func (p *processorImp) collectCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mCalls.SetName("observex_calls_total")
...
func (p *processorImp) aggregateMetrics(traces ptrace.Traces) {
	for i := 0; i < traces.ResourceSpans().Len(); i++ {
		...
		resourceAttr.PutStr(observexID, p.instanceID)
