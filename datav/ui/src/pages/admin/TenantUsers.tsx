...
const AdminTenantUsers = () => {
    ...
    const [userDetail, setUserDetail] = useState<{ user: User; teams: Team[] }>(null)
    ...
    const viewDetail = async (user: User) => {
        const res = await requestApi.get(`/user/detail?id=${user.id}&tenantId=${config.currentTenant}`)
        setUserDetail({
            user: user,
    ...
    return <>
        ...
        <Modal isOpen={isDetailOpen} onClose={() => { setUserDetail(null); onDetailClose() }}>
            ...
            {userDetail && <ModalContent>
                <ModalCloseButton />
                <ModalBody py="4">
                    <Text fontWeight={550}>User `{userDetail.user.username}` is in teams: </Text>
                    <HStack my="3">
                        {
                            userDetail.teams.map(team => <Box cursor="pointer" onClick={() => navigate(`/${config.currentTeam}/cfg/team/members`)}><ColorTag name={team.name} /></Box>)
                        }
                    </HStack>
                </ModalBody>
            </ModalContent>}
