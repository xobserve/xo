    ...
    return <>
        <Box>
            <Flex justifyContent="space-between">
                <Box></Box>
                <Button size="sm" onClick={() => navigate(`/${teamId}${ReserveUrls.New}/dashboard?team=${team.id}`)}>{t.newItem({name: t.dashboard})}</Button>
            </Flex>
                    ...
                    <Tbody>
                        {dashboards?.map(dash => {
                            return <Tr key={dash.id}>
                                ...
                                <Td>
                                    <Button variant="ghost" size="sm" px="0" onClick={() => navigate(`/${teamId}/${dash.id}`)}>{t.manage}</Button>
                                </Td>
                            </Tr>
                        })}
                    </Tbody>
