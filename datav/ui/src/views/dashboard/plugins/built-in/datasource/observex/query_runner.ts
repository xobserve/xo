...
import { $observexQueryParams } from "./store"
...
export const runQuery = async (panel: Panel, q: PanelQuery, range: TimeRange, ds: Datasource, extraParams?: Record<string,any>) => {
    ...
    const queryParams = $observexQueryParams.get()
...
export const queryobservexVariableValues = async (variable: Variable, useCurrentTimerange = true) => {
    let result = {
        error: null,
        data: null
    }
    ...
    const query:PanelQuery = {
        ...
        data:{
            [data.url]: {
                params : replaceobservexQueryWithVariables(data.params, null)
            }
        },
    }
...
export const replaceobservexQueryWithVariables = (query: PanelQuery | string, interval: string) => {
