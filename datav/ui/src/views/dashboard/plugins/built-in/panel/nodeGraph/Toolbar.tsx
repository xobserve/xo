...
export const NodeGraphToolbar = memo(
  ({ graph, dashboardId, panelId, data, onFilterRulesChange }: Props) => {
      ...
      <>
        ...
        >
          <Tooltip label='Zoom in'>
            <Box cursor='pointer' onClick={handleZoomIn}>
              <FaMinus />
            </Box>
          </Tooltip>
          <Tooltip label='Fit to canvas'>
            <Text cursor='pointer' fontWeight='600' onClick={handleFitViw}>
              FIT
            </Text>
          </Tooltip>
          <Tooltip label='Zoom out'>
            <Box cursor='pointer' onClick={handleZoomOut}>
              <FaPlus />
            </Box>
          </Tooltip>
          <Tooltip label='Fisheye magnifying,most useful when nodes squeezed together'>
            <Box color='currentcolor' cursor='pointer' onClick={toggleFishEye}>
              <FaRegCircle />
            </Box>
          </Tooltip>
          <Filter
        ...
        <Box
          className='nodegraph-menutip bordered'
          opacity={menuTip.opacity}
          position='absolute'
          right='25px'
          width='fit-content'
          top='2px'
