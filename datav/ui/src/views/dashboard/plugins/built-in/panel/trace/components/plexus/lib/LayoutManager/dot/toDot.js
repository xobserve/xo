...
function makeGraphWrapper(options) {
  const { nodesep, rankdir, ranksep, sep, shape, splines } = {
    ...DEFAULT_GRAPH_ATTRS,
    ...options,
  }
  return `digraph G {
  graph[nodesep=${nodesep.toFixed(
    3,
  )}, rankdir=${rankdir}, ranksep=${ranksep.toFixed(3)}, sep=${sep.toFixed(
    3,
  )}, splines=${splines}];
...
function makeNode(v) {
  ...
  return `"${vertex.key}" [height=${height.toFixed(5)},width=${width.toFixed(
    5,
  )}${pos}];`
}
...
export default function toDot(edges, vertices, options) {
  const bidirectional = []
  ...
  const bidirStrs = bidirectional.map((edge) =>
    makeEdge(edge.from, edge.to, true),
  )
  const edgeStrs = []
