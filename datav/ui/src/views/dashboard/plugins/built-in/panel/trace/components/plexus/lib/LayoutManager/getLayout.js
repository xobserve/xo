...
import { EWorkerPhase } from './types'
var EValidity = /*#__PURE__*/ (function (EValidity) {
  EValidity['Ok'] = 'Ok'
  EValidity['Warn'] = 'Warn'
...
function getVerticesValidity(input, output) {
  ...
  for (let i = 0; i < output.length; i++) {
    ...
    if (
      !isCloseEnough(src.height, height) ||
      !isCloseEnough(src.width, width)
    ) {
      return {
        validity: EValidity.Error,
...
export default function getLayout(phase, inEdges, inVertices, layoutOptions) {
  ...
  const { edges, graph, vertices } = convPlain(
    plainOut,
    phase !== EWorkerPhase.Positions,
  )
