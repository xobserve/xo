        ...
        <Center height='100%'>
          <VStack>
            ...
            <Text className='color-text'>
              Try to change to Testdata datasource, then look into its data
              format in Panel Debug
            </Text>
          </VStack>
        </Center>
      ) : (
...
const NodeGrapPanel = ({
  ...
  const defaultEdgeLabelCfg = getDefaultEdgeLabel(
    colorMode,
  ...
  const donutColors = {}
  panel.plugins.nodeGraph.node.donutColors.forEach((item) => {
    donutColors[item.attr] = item.color
  })
  ...
  const highlightNodeNames = useMemo(() => {
    ...
    if (panel.plugins.nodeGraph.node.enableHighlight) {
      const filterHighlight = genDynamicFunction(
        panel.plugins.nodeGraph.node.highlightNodesByFunc,
      )
  ...
  useEffect(() => {
    ...
    registerCustomNode(
      ...
      paletteColorNameToHex(panel.plugins.nodeGraph.node.highlightColor),
      highlightNodeNames,
    )
    if (graph) {
    ...
    panel.plugins.nodeGraph.node.enableHighlight,
    panel.plugins.nodeGraph.node.highlightNodes,
    panel.plugins.nodeGraph.node.highlightColor,
    highlightNodeNames,
  ])
      ...
      const gh = new G6.Graph({
        ...
        defaultEdge: {
          ...
          style: {
            ...
            // offset: 30,
            endArrow:
              panel.plugins.nodeGraph.edge.arrow == 'default'
                ? true
                : {
                    ...
                    fill:
                      colorMode == 'light'
                        ? paletteColorNameToHex(
                            panel.plugins.nodeGraph.edge.color.light,
                            colorMode,
                          )
                        : paletteColorNameToHex(
                            panel.plugins.nodeGraph.edge.color.dark,
                            colorMode,
                          ),
                  },
            // lineAppendWidth: 2,
        ...
        defaultNode: {
          ...
          style: {
            lineWidth: 0,
            fill: 'transparent',
            stroke: paletteColorNameToHex(
              panel.plugins.nodeGraph.node.borderColor,
              colorMode,
            ),
          },
        ...
        nodeStateStyles: getDefaultNodeStyle(
          panel.plugins.nodeGraph,
          colorMode,
        ),
        edgeStateStyles: getDefaultEdgeStyle(
          panel.plugins.nodeGraph,
          colorMode,
        ),
      })
      ...
      g1.on('node:mouseenter', (evt) => {
        ...
        item.getEdges().forEach((edge) => {
          g1.updateItem(edge, {
            ...
            labelCfg: getActiveEdgeLabelCfg(
              newestColorMode,
              panel.plugins.nodeGraph,
            ),
          })
    ...
    <>
      ...
      {activeEdge && (
        <Box
          ...
          position='absolute'
          left={
            panel.plugins[PanelTypeNodeGraph].edge.detailPos == 'edge'
              ? activeEdge._cfg.model.startPoint?.x +
                (activeEdge._cfg.model.endPoint?.x -
                  activeEdge._cfg.model.startPoint?.x) /
                  2
              : '0'
          }
          top={
            panel.plugins[PanelTypeNodeGraph].edge.detailPos == 'edge'
              ? activeEdge._cfg.model.startPoint?.y +
                (activeEdge._cfg.model.endPoint?.y -
                  activeEdge._cfg.model.startPoint?.y) /
                  2
              : '0'
          }
        ...
        >
          <Flex gap='3'>
            <ColorKV
              k={activeEdge._cfg.model.label}
              v={'callsCount/error/p99'}
              renderString={false}
            />
          </Flex>
          <Flex gap='3'>
            <ColorKV
              k='source'
              v={activeEdge._cfg.model.source}
              renderString={false}
            />
          </Flex>
          <Flex gap='3'>
            <ColorKV
              k='target'
              v={activeEdge._cfg.model.target}
              renderString={false}
            />
          </Flex>
          {Object.entries(activeEdge._cfg.model.data).map((kv) => (
            <Flex gap='3'>
