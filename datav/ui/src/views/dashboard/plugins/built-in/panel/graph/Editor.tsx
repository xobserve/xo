    ...
    return (<>
        <PanelAccordion title="Tooltip">
            <PanelEditItem title={t.mode}>
                <RadionButtons options={[{ label: "Single", value: "single" }, { label: "All", value: "all" }, { label: "Hidden", value: "hidden" }]} value={panel.plugins.graph.tooltip.mode} onChange={v => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title="Legend">
            <PanelEditItem title={t.mode}>
                <RadionButtons options={[{ label: "Table", value: "table" }, { label: "Hidden", value: "hidden" }]} value={panel.plugins.graph.legend.mode} onChange={v => onChange((panel: GraphPanel) => {
            ...
            {panel.plugins.graph.legend.mode != 'hidden' && <>
                <PanelEditItem title={t1.placement}>
                    <RadionButtons options={[{ label: "Bottom", value: "bottom" }, { label: "Right", value: "right" }]} value={panel.plugins.graph.legend.placement} onChange={v => onChange((panel: GraphPanel) => {
                        panel.plugins.graph.legend.placement = v
                    })} />
                </PanelEditItem>
                {panel.plugins.graph.legend.placement == "right" && <PanelEditItem title={t1.width}>
                    <EditorNumberItem value={panel.plugins.graph.legend.width} min={100} step={50} onChange={v => onChange((panel: GraphPanel) => { panel.plugins.graph.legend.width = (v <= 100 ? 100 : v) })} />
                </PanelEditItem>}
                {panel.plugins.graph.legend.placement == "right" && <PanelEditItem title={t1.nameWidth} desc={t1.nameWidthTips}>
                    <EditorInputItem value={panel.plugins.graph.legend.nameWidth} onChange={v => onChange((panel: GraphPanel) => {
                        panel.plugins.graph.legend.nameWidth = v
                    })} />
                </PanelEditItem>}
                <PanelEditItem title={t1.values} desc={t1.valuesTips}>
                    <PopoverSelect value={panel.plugins.graph.legend.valueCalcs} isMulti options={Object.keys(ValueCalculationType).map(k => ({ label: k, value: k }))} onChange={v => onChange((panel: GraphPanel) => {
                        panel.plugins.graph.legend.valueCalcs = v as any
                    })} />
                </PanelEditItem>
                <PanelEditItem title={t1.showValuesName}>
                    <Switch defaultChecked={panel.plugins.graph.legend.showValuesName} onChange={e => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title={t1.graphStyles}>
            <PanelEditItem title={t.type}>
                <RadionButtons options={[{ label: "Lines", value: "lines" }, { label: "Bars", value: "bars" }, { label: "points", value: "points" }]} value={panel.plugins.graph.styles.style} onChange={v => onChange((panel: GraphPanel) => {
            ...
            </PanelEditItem>
            {
                panel.plugins.graph.styles.style == "bars" && <>
                    <PanelEditItem title={t1.barRadius}>
                        <EditorNumberItem value={panel.plugins.graph.styles.barRadius} min={0} max={0.5} step={0.1} onChange={v => onChange((panel: GraphPanel) => {
                            panel.plugins.graph.styles.barRadius = v
                            dispatch(PanelForceRebuildEvent + panel.id)
                        })} />
                    </PanelEditItem>
                    <PanelEditItem title={t1.barGap}>
                        <EditorNumberItem value={panel.plugins.graph.styles.barGap} min={0} max={100} step={5} onChange={v => onChange((panel: GraphPanel) => {
            ...
            }
            {
                <>
                    <PanelEditItem title={t1.lineWidth}>
                        <EditorSliderItem value={panel.plugins.graph.styles.lineWidth} min={0} max={10} step={1} onChange={v => onChange((panel: GraphPanel) => {
                            panel.plugins.graph.styles.lineWidth = v
                        })} />
                    </PanelEditItem>
                    <PanelEditItem title={t1.gradient}>
                        <RadionButtons options={[{ label: "None", value: "none" }, { label: "Opacity", value: "opacity" }]} value={panel.plugins.graph.styles.gradientMode} onChange={v => onChange((panel: GraphPanel) => {
                    ...
                    <PanelEditItem title={t1.showPoints}>
                        <RadionButtons options={[{ label: "Auto", value: "auto" }, { label: "Always", value: "always" }, { label: "Never", value: "never" }]} value={panel.plugins.graph.styles.showPoints} onChange={v => onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title={t1.pointSize}>
                <EditorSliderItem value={panel.plugins.graph.styles.pointSize} min={1} max={20} step={1} onChange={v => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.styles.pointSize = v
                })} />
            </PanelEditItem>

            <PanelEditItem title={t1.connectNull}>
                <Switch defaultChecked={panel.plugins.graph.styles.connectNulls} onChange={e => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.styles.connectNulls = e.currentTarget.checked
                })} />
            </PanelEditItem>
            <PanelEditItem title="Stack">
                <Switch defaultChecked={panel.plugins.graph.styles.enableStack} onChange={e => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title={t.axis}>
            ...
            <PanelEditItem title={t1.showGrid}>
                <RadionButtons options={[{ label: "Show", value: true }, { label: "Hidden", value: false }]} value={panel.plugins.graph.axis.showGrid} onChange={v => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.axis.showGrid = v
                })} />
            </PanelEditItem>
            <PanelEditItem title="Show X axis">
                <Switch defaultChecked={panel.plugins.graph.axis.showX} onChange={e => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.axis.showX = e.currentTarget.checked
                })} />
            </PanelEditItem>
            <PanelEditItem title="Show Y axis">
                <Switch defaultChecked={panel.plugins.graph.axis.showY} onChange={e => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.axis.showY = e.currentTarget.checked
                })} />
            </PanelEditItem>
            <PanelEditItem title={t.scale}>
                <RadionButtons options={[{ label: "Linear", value: "linear" }, { label: "Log", value: "log" }]} value={panel.plugins.graph.axis.scale} onChange={v => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.axis.scale = v
                })} />
            </PanelEditItem>
            {panel.plugins.graph.axis.scale == "log" && <PanelEditItem title="Scale Base">
                <RadionButtons options={[{ label: "Base 2", value: "2" }, { label: "Base 10", value: "10" }]} value={panel.plugins.graph.axis.scaleBase as any} onChange={v => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title="Value">
            <PanelEditItem title={t.unit}>
                <UnitPicker value={panel.plugins.graph.value} onChange={
                    (v: Units) => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title="Thresholds">
            <ThresholdEditor value={panel.plugins.graph.thresholds} onChange={(v) => onChange((panel: GraphPanel) => {
        ...
        <PanelAccordion title={t1.alertCorrelation}>
            <PanelEditItem title="Enable" desc={t1.alertCorrelationTips}>
                <Switch defaultChecked={panel.plugins.graph.enableAlert} onChange={e => onChange((panel: GraphPanel) => {
                    panel.plugins.graph.enableAlert = e.currentTarget.checked
                })} />
            </PanelEditItem>
        </PanelAccordion>
        {panel.plugins.graph.enableAlert && <AlertFilterEditor panel={panel as any} onChange={onChange} filter={panel.plugins.graph.alertFilter} />}
    </>
    )
})
