...
const EditPanelQuery = (props: Props) => {
  ...
  const selectDatasource = (id) => {
    ...
    onChange((panel: Panel) => {
      ...
      if (panel.datasource.type == newDs.type) {
        panel.datasource = { ...panel.datasource, id: id }
      } else {
        panel.datasource = { ...initDatasource(), id: id, type: newDs.type }
      }
    })
  }
  ...
  const plugin =
    builtinDatasourcePlugins[currentDatasource?.type] ??
    externalDatasourcePlugins[currentDatasource?.type]
      ...
      <Box p='2' borderRadius='0' height='100%'>
        <Flex justifyContent='space-between' alignItems='start'>
          <HStack>
            ...
            <Box width='200px'>
              <DatasourceSelect
                value={panel.datasource.id}
                onChange={selectDatasource}
                variant='unstyled'
              />
            </Box>
            {!isNumber(panel.datasource.id) && (
              <Text textStyle='annotation'>
                current: {currentDatasource?.name}
              </Text>
            )}
            {docs && docs.length > 0 && (
              <Tooltip label={`View ${currentDatasource?.type} docs`}>
                <Box opacity={0.8} cursor='pointer' onClick={onDocsOpen}>
                  <FaBookOpen
                    className={colorMode == 'light' ? 'color-text' : null}
                  />
                </Box>
              </Tooltip>
            )}
        ...
        <VStack alignItems='left' mt='3' spacing='2'>
          {panel.datasource.queries?.map((query, index) => {
            return (
              <Box key={index} className='bordered-top' p='1' pb='2'>
                <Flex justifyContent='space-between' py='0' px='2' mb='1'>
                  <Text className='color-text'>
                    {String.fromCharCode(query.id)}
                  </Text>
                  <HStack
                  ...
                  >
                    {query.visible ? (
                      <FaEye
                        cursor='pointer'
                        onClick={() => onVisibleChange(query.id, false)}
                      />
                    ) : (
                      <FaEyeSlash
                        cursor='pointer'
                        onClick={() => onVisibleChange(query.id, true)}
                      />
                    )}
                    <FaTrashAlt
                      cursor='pointer'
                ...
                </Flex>
                {
                  <Box px='2' pt='2'>
                    <CustomQueryEditor
                      key={panel.datasource.id + query.id}
                      panel={panel}
                      query={query}
                      selected={panel.datasource}
                      dsType={currentDatasource?.type}
                      onChange={onChange}
                    />
                  </Box>
                }
...
const DatasourceQueryOption = ({ panel, onChange }: Props) => {
  ...
  return (
    <VStack alignItems='end' mt='8px'>
      ...
      {expanded && (
        <FormSection mt='1' position='relative'>
          <FormItem
            size='sm'
            labelWidth='170px'
            title={t1.maxDataPoints}
            desc={t1.maxDataPointsTips}
          >
            <Box width='100px'>
              <EditorNumberItem
                min={1}
                max={2000}
                step={20}
                value={panel.datasource.queryOptions.maxDataPoints}
                onChange={(v) => {
                  onChange((panel: Panel) => {
          ...
          <FormItem
            title={t1.minInterval}
            labelWidth='170px'
            size='sm'
            desc={t1.minIntervalTips}
          >
            <Box width='100px'>
              <EditorInputItem
                value={panel.datasource.queryOptions.minInterval}
                onChange={(v) => {
                  onChange((panel: Panel) => {
          ...
          <FormItem
            labelWidth='170px'
            size='sm'
            title={t1.finalInterval}
            desc={t1.finalIntervalTips}
            alignItems='center'
          >
            <Text width='100px' pl='2' fontSize='0.9rem'>
              {
                calculateInterval(
                  getCurrentTimeRange(),
