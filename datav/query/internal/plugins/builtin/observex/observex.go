...
import (
	...
	"github.com/DataObserve/observex/query/internal/plugins/builtin/observex/api"
...
/* Query plugin for observex observability*/

var datasourceName = "observex"

type observexPlugin struct{}

var conns = make(map[int64]ch.Conn)
var connsLock = &sync.Mutex{}

func (p *observexPlugin) Query(c *gin.Context, ds *models.Datasource) models.PluginResult {
	query := c.Query("query")
	conn, ok := conns[ds.Id]
	if !ok {
		var err error
...
func (p *observexPlugin) TestDatasource(c *gin.Context) models.PluginResult {
	return pluginUtils.TestClickhouseDatasource(c)
}

func init() {
	// register datasource
	models.RegisterPlugin(datasourceName, &observexPlugin{})
}
