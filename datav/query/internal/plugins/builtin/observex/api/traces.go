...
func GetTraces(c *gin.Context, ds *models.Datasource, conn ch.Conn, params map[string]interface{}) models.PluginResult {
	...
	var service string
	if service0 == "" {
		service = observexutils.GetValueFromParams(params, "service")
	} else {
		service = service0
	}
	...
	traceIndexes := make([]*observexmodels.TraceIndex, 0)
	...
	if onlyChart != "true" {
		...
		traceIndexMap0 := make(map[string][]*observexmodels.TraceIndex, 0)
		traceIDList := make([]string, 0)
		for rows.Next() {
			var traceIndex observexmodels.TraceIndex
		...
		traceIndexMap := make(map[string]*observexmodels.TraceIndex, 0)
		...
		query = fmt.Sprintf("select traceId,serviceName,hasError,count(spanId) from %s.%s where traceId in ('%s') GROUP by traceId,serviceName,hasError", config.Data.Observability.DefaultTraceDB, observexmodels.DefaultTraceIndexTable, strings.Join(traceIDList, "','"))
		rows, err = conn.Query(c.Request.Context(), query)
		if err != nil {
		...
		traceServicesMap := make(map[string]map[string]*observexmodels.TraceServiceIndex)
		for rows.Next() {
			...
			if !ok {
				var s = &observexmodels.TraceServiceIndex{
					Name:     serviceName,
					NumSpans: numSpans,
					Errors:   0,
				}
				...
				trace = map[string]*observexmodels.TraceServiceIndex{
					serviceName: s,
				}
		...
		for traceID, serviceMap := range traceServicesMap {
			...
			if ok {
				traceIndex.Services = make([]*observexmodels.TraceServiceIndex, 0)
...
func GetTrace(c *gin.Context, ds *models.Datasource, conn ch.Conn, params map[string]interface{}) models.PluginResult {
	...
	query := fmt.Sprintf("SELECT startTime, traceId, model FROM %s.%s WHERE traceId='%s'", config.Data.Observability.DefaultTraceDB, observexmodels.DefaultTraceSpansTable, traceID)
	// query traceIDs
	rows, err := conn.Query(c.Request.Context(), query)
	if err != nil {
		logger.Warn("Error Query trace ids", "query", query, "error", err)
...
func GetTraceTagKeys(c *gin.Context, ds *models.Datasource, conn ch.Conn, params map[string]interface{}) models.PluginResult {
	...
	if service == "" {
		serviceI := observexutils.GetValueListFromParams(params, "service")
		if serviceI != nil {
	...
	query := fmt.Sprintf("SELECT DISTINCT tagKey,tagType,dataType,isColumn FROM %s.%s WHERE (%s) OR isColumn=true", config.Data.Observability.DefaultTraceDB, observexmodels.DefaultSpanAttributeKeysTable, domainQuery)
	// query traceIDs
	rows, err := conn.Query(c.Request.Context(), query)
	if err != nil {
