CREATE TABLE IF NOT EXISTS observex_traces.dependency_graph_minutes ON CLUSTER cluster (
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.dependency_graph_minutes_service_calls_mv ON CLUSTER cluster
TO observex_traces.dependency_graph_minutes AS
SELECT
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.dependency_graph_minutes_db_calls_mv ON CLUSTER cluster
TO observex_traces.dependency_graph_minutes AS
SELECT
    serviceName as src,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.dependency_graph_minutes_messaging_calls_mv ON CLUSTER cluster
TO observex_traces.dependency_graph_minutes AS
SELECT
    serviceName as src,
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_dependency_graph_minutes ON CLUSTER cluster AS observex_traces.dependency_graph_minutes
ENGINE = Distributed("cluster", "observex_traces", dependency_graph_minutes, cityHash64(rand()));