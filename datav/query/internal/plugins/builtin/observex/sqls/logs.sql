CREATE TABLE IF NOT EXISTS observex_logs.logs ON CLUSTER cluster  (
...
CREATE TABLE IF NOT EXISTS observex_logs.distributed_logs ON CLUSTER cluster  AS observex_logs.logs
ENGINE = Distributed("cluster","observex_logs", logs, cityHash64(id));


CREATE TABLE IF NOT EXISTS observex_logs.logs_atrribute_keys ON CLUSTER cluster (
name String,
datatype String
)ENGINE = ReplacingMergeTree
ORDER BY (name, datatype);

CREATE TABLE IF NOT EXISTS observex_logs.distributed_logs_atrribute_keys  ON CLUSTER cluster AS observex_logs.logs_atrribute_keys
ENGINE = Distributed("cluster", "observex_logs", logs_atrribute_keys, cityHash64(datatype));

CREATE TABLE IF NOT EXISTS observex_logs.logs_resource_keys ON CLUSTER cluster (
name String,
datatype String
)ENGINE = ReplacingMergeTree
ORDER BY (name, datatype);

CREATE TABLE IF NOT EXISTS observex_logs.distributed_logs_resource_keys  ON CLUSTER cluster AS observex_logs.logs_resource_keys
ENGINE = Distributed("cluster", "observex_logs", logs_resource_keys, cityHash64(datatype));

CREATE TABLE IF NOT EXISTS observex_logs.log_tag_attributes ON CLUSTER cluster (
...
CREATE TABLE IF NOT EXISTS observex_logs.distributed_log_tag_attributes ON CLUSTER cluster AS observex_logs.log_tag_attributes
ENGINE = Distributed("cluster", "observex_logs", log_tag_attributes, rand());


CREATE MATERIALIZED VIEW IF NOT EXISTS  atrribute_keys_string_final_mv ON CLUSTER cluster TO observex_logs.logs_atrribute_keys AS
SELECT
distinct arrayJoin(attributes_string_key) as name, 'String' datatype
FROM observex_logs.logs
ORDER BY name;

CREATE MATERIALIZED VIEW IF NOT EXISTS  atrribute_keys_int64_final_mv ON CLUSTER cluster  TO observex_logs.logs_atrribute_keys AS
SELECT
distinct arrayJoin(attributes_int64_key) as name, 'Int64' datatype
FROM observex_logs.logs
ORDER BY  name;

CREATE MATERIALIZED VIEW IF NOT EXISTS  atrribute_keys_float64_final_mv ON CLUSTER cluster TO observex_logs.logs_atrribute_keys AS
SELECT
distinct arrayJoin(attributes_float64_key) as name, 'Float64' datatype
FROM observex_logs.logs
ORDER BY  name;

CREATE MATERIALIZED VIEW IF NOT EXISTS  resource_keys_string_final_mv ON CLUSTER cluster  TO observex_logs.logs_resource_keys AS
SELECT
