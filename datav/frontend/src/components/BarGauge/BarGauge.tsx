...
const BarGauge = (props: Props) => {
  ...
  return (
    <Box position='relative' width='100%' height='100%'>
      <Stack
        alignItems={orientation == 'horizontal' ? 'left' : 'top'}
        position='absolute'
        top='0'
        left='0'
        right='0'
          ...
          const lcdSize =
            orientation == 'horizontal'
              ...
              : height -
                titleHeight -
                20 -
                (showMin || showMax ? minMaxHeight : 0)
          const lcdCellCount = Math.floor(lcdSize / lcdCellWidth!)
          const lcdCellSize = Math.floor(
            (lcdSize - lcdCellSpacing * lcdCellCount) / lcdCellCount,
          )
          const gap =
            lcdSize - lcdCellCount * (lcdCellSize + lcdCellSpacing) + 1
              ...
              >
                {
                  <>
                    ...
                    <Flex
                      ...
                      width='100%'
                      height={
                        v.title ? `calc(100% - ${titleHeight}px)` : '100%'
                      }
                      borderRadius={borderRadius}
                    >
                      <Box
                        width={`calc(100% - ${textWidth}px - 20px)`}
                        position='relative'
                        height='100%'
                        bg={
                          showUnfilled
                        ...
                        {mode == 'lcd' ? (
                          ...
                          <Box
                            bg={
                              color == 'transparent'
                                ? color
                                : alpha(color, fillOpacity)
                            }
                            width={`${Math.min(
                              ((v.value - v.min) * 100) / (v.max - v.min),
                              100,
                            )}%`}
                            height='100%'
                            borderRadius={borderRadius}
                      ...
                      <Box width={textWidth} maxW='40%'>
                        {showMin && (
                          <Text fontSize='0.6rem' opacity='0.4'>{`${formatUnit(
                            v.min,
                            v.units,
                            v.decimal,
                          )}`}</Text>
                        )}
                        <Text fontSize={`${textSize}px`} color={color}>
                          {v.text}
                        </Text>
                        {showMax && (
                          <Text fontSize='0.6rem' opacity='0.4'>{`${formatUnit(
              ...
              <CustomScrollbar hideHorizontalTrack>
                ...
                >
                      ...
                      <VStack width='100%' height={lcdSize} alignItems='left'>
                        {Array.from({ length: lcdCellCount }, (_, i) => {
                          ...
                          return (
                            <Box
                              position='absolute'
                              bottom={`${i * (lcdCellSize + lcdCellSpacing)}px`}
                              key={i}
                              height={`${lcdCellSize}px`}
                              width='100%'
                              bg={cellColor}
                              borderRadius='2px'
                            ></Box>
                  ...
                  <Flex justifyContent='space-between' alignItems='center'>
                    {showMin ? (
                      <Text fontSize='0.6rem' opacity='0.4'>
                        {`${formatUnit(v.min, v.units, v.decimal)}`}{' '}
                      </Text>
                    ) : (
                      <Box></Box>
                    )}
                    {showMax ? (
                      <Text fontSize='0.6rem' opacity='0.4'>
                        {' '}
                        {`${formatUnit(v.max, v.units, v.decimal)}`}
                      </Text>
                    ) : (
                      <Box></Box>
                    )}
