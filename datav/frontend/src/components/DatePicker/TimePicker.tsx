      ...
      {tempRange && (
        <Flex alignItems='top' justifyContent='space-between' gap='6'>
          ...
          <VStack alignItems='left' spacing={4}>
            ...
            <Box>
              ...
              <HStack>
                <EditorInputItem
                  ...
                  value={
                    tempRange.startRaw.toString().startsWith('now-')
                      ? tempRange.startRaw
                      : dateTimeFormat(tempRange.start)
                  }
            ...
            <Box>
              ...
              <HStack>
                <EditorInputItem
                  ...
                  value={
                    tempRange.endRaw.toString().startsWith('now')
                      ? tempRange.endRaw
                      : dateTimeFormat(tempRange.end)
                  }
            ...
            <Button size='sm' onClick={() => applyTimeRange(tempRange)}>
              {t1.apply}
            </Button>
          </VStack>
          <Box p='2' pt='0'>
            <Center>
              <Text>{t1.quickSelect}</Text>
            </Center>
            <VStack
              spacing={3}
              pt={4}
            ...
            >
              {quickOptions.map((o) => (
                <Button
                  size='md'
                  key={o.value}
                  onClick={() => setQuickTime(o)}
                  colorScheme='gray'
                  variant={
                    range.startRaw == o.raw && range.endRaw == 'now'
                      ? 'solid'
                      : 'ghost'
                  }
                  borderRadius='0'
  ...
  switch (unit) {
    case 'y':
      if (roundUp) {
        ...
        dateTime
          .subtract((dateTime.month() - fyStartMonth + 12) % 12, 'M')
          .startOf('M')
      }
      return dateTime
    case 'Q':
      if (roundUp) {
        ...
        dateTime
          .subtract((dateTime.month() - fyStartMonth + 3) % 3, 'M')
          .startOf('M')
      }
      return dateTime
    default:
