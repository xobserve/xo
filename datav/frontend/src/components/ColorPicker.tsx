            ...
            <TabPanels>
              <TabPanel pt='1'>
                <VStack alignItems='left' spacing={1} px='1'>
                  {(colorMode == 'light' ? lightPalettes : darkPalettes).map(
                    (palette) => (
                      <Flex justifyContent='space-between' alignItems='center'>
                        <Text fontSize='0.9rem'>
                          {upperFirst(palette.name)}
                        </Text>
                        <HStack spacing={3}>
                ...
                <HStack mt='3' spacing={4}>
                  ...
                  <HStack>
                    ...
                    <Box
                      ...
                      className='bordered'
                      borderColor={
                        props.color === 'inherit' ? colors[0] : 'transparent'
                      }
                    >
                      <Box
                        cursor='pointer'
...
const Color = ({ color, currentColor, onChange }) => {
  return (
    <Box
      p='1px'
      borderRadius={4}
      border={`1.5px solid ${currentColor == color ? color : 'transparent'}`}
    >
      <Box
        cursor='pointer'
        borderRadius='50%'
...
const Palette = ({ name, color, currentColor, onChange, isCenter }) => {
  return (
    <Box
      p='1px'
      borderRadius={4}
      border={`1.5px solid ${currentColor == name ? color : 'transparent'}`}
    >
      <Box
        cursor='pointer'
        borderRadius='50%'
        width={isCenter ? '25px' : '17px'}
        height={isCenter ? '25px' : '17px'}
        display='block'
        bg={color}
