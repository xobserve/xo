...
import { concat, isArray, isEmpty, orderBy } from 'lodash'
    ...
    <>
      ...
      {variable && (
        <EditVariable
          ...
          isOpen={isOpen}
          onSubmit={editMode ? editVariable : addVariable}
          isGlobal
        />
      )}
      {variables && (
      ...
      {variables.length > 0 ? (
        <TableContainer>
          <Table variant='simple' className='color-border-table'>
            ...
            <Tbody>
              {orderBy(variables, ['sortWeight', 'name'], ['desc', 'asc']).map(
                (variable, i) => {
                  return (
                    <Tr
                      key={variable.name}
...
export const EditVariable = ({
  ...
  const queryValues = (v0?) => {
    queryVariableValues(v0 ?? v, datasources).then((result) =>
      setVariableValues(result.data ?? []),
    )
  }
  ...
  const externalDsList = Object.entries(externalDatasourcePlugins).filter(
    ([k, v]) => v.variableEditor,
  )
  const builtinDsList = Object.entries(builtinDatasourcePlugins).filter(
    ([k, v]) => v.variableEditor,
  )
  ...
  const PluginEditor = dsPluginList.find(
    ([k, v]) => k == currentDatasource?.type,
  )?.[1]?.variableEditor
      ...
      <Modal isOpen={isOpen} onClose={onClose} size='full'>
        ...
        <ModalContent w='600px'>
              ...
              >
                <FormSection title={t.basicSetting}>
                  ...
                  <FormItem title={t1.refresh}>
                    <RadionButtons
                      options={Object.keys(VariableRefresh).map((k) => ({
                        label: t1[k],
                        value: VariableRefresh[k],
                      }))}
                      value={variable.refresh}
                      onChange={(v) => setVariable({ ...variable, refresh: v })}
                    />
                  </FormItem>

                  <FormItem title={t1.multiValue} alignItems='center'>
                    <Switch
                      defaultChecked={variable.enableMulti}
                      onChange={(e) =>
                        setVariable({
                          ...variable,
                          enableMulti: e.currentTarget.checked,
                        })
                      }
                    />
                  </FormItem>
                  ...
                  <FormItem title={t1.allValue} alignItems='center'>
                    <Switch
                      defaultChecked={variable.enableAll}
                      onChange={(e) =>
                        setVariable({
                          ...variable,
                          enableAll: e.currentTarget.checked,
                        })
                      }
                    />
                  </FormItem>
                  ...
                  <FormItem
                    title={t.sortWeight}
                    desc='Larger weight means higher sort priority when being display in dashboard'
                  >
                    <EditorNumberItem
                      size='md'
                      placeholder='auto'
                      key={variable.sortWeight}
                      value={variable.sortWeight}
                      min={0}
                      max={100}
                      step={1}
                      onChange={(v) => {
                ...
                <FormSection title={t.query}>
                  <FormItem title={t1.queryType}>
                    <RadionButtons
                      options={Object.keys(VariableQueryType).map((k) => ({
                        label: t[VariableQueryType[k]],
                        value: VariableQueryType[k],
                      }))}
                      value={variable.type}
                      onChange={(v) => {
                  ...
                  {variable.type == VariableQueryType.Custom && (
                    <FormItem title={t1.queryValue}>
                      <Input
                        width='400px'
                        placeholder={t1.valueTips}
                ...
                <FormSection title={t1.defaultValue} desc={t1.defaultValueTips}>
                  <Input
                    width='400px'
                    placeholder={'input a default value'}
                    value={variable.default}
                    onChange={(e) => {
                      setVariable({
                        ...variable,
                        default: e.currentTarget.value?.trim(),
                      })
                    }}
                  />
                </FormSection>
                <FormSection title={t1.varValues}>
                  <Box pt='1'>
                    {isArray(filterValues) &&
                      filterValues.slice(0, displayCount).map((v) => (
                        <Tag key={v} size='sm' variant='outline' ml='1'>
                          {v}
                        </Tag>
                      ))}
                  </Box>
                  {filterValues?.length > displayCount && (
                    <Button
                      mt='2'
                      size='sm'
                      colorScheme='gray'
          ...
          <ModalFooter>
            ...
            <Button variant='ghost' onClick={() => onSubmit(variable)}>
              {t.submit}
            </Button>
          </ModalFooter>
        </ModalContent>
