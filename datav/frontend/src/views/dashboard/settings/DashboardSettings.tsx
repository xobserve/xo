    ...
    <>
      <Tooltip label={t1.dashSettings}>
        <Box>
          <IconButton
            onClick={() =>
              addParamToUrl({ settings: DashboardSettingType.General })
            }
            variant='ghost'
          >
            <FaCog />
        ...
        <ModalContent>
          ...
          <ModalBody p={isLargeScreen ? 2 : 1}>
            ...
            <Tabs
              ...
              mt='7'
              defaultIndex={toNumber(settings)}
              onChange={(index) => addParamToUrl({ settings: index })}
            >
              <TabList
                pr={isLargeScreen ? 2 : 0}
                width={isLargeScreen ? '200px' : '70px'}
              >
                <Tab>{t.general}</Tab>
                <Tab>{t.styles}</Tab>
                <Tab>{t.variable}</Tab>
