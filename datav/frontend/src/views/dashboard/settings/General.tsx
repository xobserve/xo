    ...
    <>
      ...
      >
        <FormSection title={t.basicSetting} spacing={1}>
          ...
          <FormItem
            title={t1.visibleTo}
            desc='Controls who can see this dashboard'
          >
            <RadionButtons
              options={[
          ...
          <FormItem title={t1.tootip} desc={t1.tootipTips} alignItems='center'>
            <Switch
              isChecked={dashboard.data.sharedTooltip}
              onChange={(e) =>
                onChange((draft: Dashboard) => {
                  draft.data.sharedTooltip = e.currentTarget.checked
                })
              }
            />
          </FormItem>
          ...
          <FormItem title={t1.hideVars}>
            <Input
              value={hidingVars}
              onChange={(e) => setHidingVars(e.currentTarget.value)}
              onBlur={() =>
                onChange((draft: Dashboard) => {
                  draft.data.hidingVars = hidingVars
                })
              }
              placeholder={t1.hideVarsTips}
            />
          </FormItem>
            ...
            <HStack width='100%'>
              {dashboard.tags.map((t) => (
                <ColorTag
                  ...
                  onRemove={(_) => {
                    onChange((draft: Dashboard) => {
                      draft.tags.splice(draft.tags.indexOf(t), 1)
                    })
                  }}
                />
              ))}
          ...
          <FormItem title={t1.panelLayout} desc={t1.panelLayoutTips}>
            <Select
              value={dashboard.data.layout}
              onChange={(e) => {
                const v = e.currentTarget.value
                onChange((draft: Dashboard) => {
                  draft.data.layout = v as DashboardLayout
                })
              }}
            >
              {Object.keys(DashboardLayout).map((k) => (
                <option value={[DashboardLayout[k]]}>
                  {DashboardLayout[k]}
                </option>
              ))}
            </Select>
          ...
          <FormItem
            title={t1.panelOverlap}
            desc={t1.panelOverlapTips}
            alignItems='center'
          >
            <Switch
              isChecked={dashboard.data.allowPanelsOverlap}
              onChange={(e) =>
                onChange((draft: Dashboard) => {
                  draft.data.allowPanelsOverlap = e.currentTarget.checked
                })
              }
            />
          </FormItem>
          <FormItem
            title={t1.hiddenPanel}
            desc={t1.hiddenPanelTips}
            alignItems='center'
          >
            <HStack>
              {dashboard.data.hiddenPanels.map((id) => (
                <Tag colorScheme='gray'>
                  <TagLabel>
                    {id} / {dashboard.data.panels.find((p) => p.id == id).title}
                  </TagLabel>
                  <TagCloseButton
                    onClick={() => {
                      onChange((draft: Dashboard) => {
                        draft.data.hiddenPanels.splice(
                          draft.data.hiddenPanels.indexOf(id),
                          1,
                        )
                      })
                    }}
        ...
        <FormSection title={t1.loadData}>
          <FormItem
            title={t1.lazyRender}
            desc={t1.lazyRenderTips}
            alignItems='center'
          >
            <Switch
              isChecked={dashboard.data.lazyLoading}
              onChange={(e) =>
                onChange((draft: Dashboard) => {
                  draft.data.lazyLoading = e.currentTarget.checked
                })
              }
            />
          </FormItem>
        ...
        <FormSection title={t1.saveDash}>
          <FormItem
            title={t1.savePromt}
            desc={t1.savePromtTips}
            alignItems='center'
          >
            <Switch
              isChecked={dashboard.data.enableUnsavePrompt}
              onChange={(e) =>
                onChange((draft: Dashboard) => {
                  draft.data.enableUnsavePrompt = e.currentTarget.checked
                })
              }
            />
          </FormItem>
          ...
          <FormItem
            title={t1.autoSave}
            desc={t1.autoSaveTips}
            alignItems='center'
          >
            <Switch
              isChecked={dashboard.data.enableAutoSave}
              onChange={(e) =>
                onChange((draft: Dashboard) => {
                  draft.data.enableAutoSave = e.currentTarget.checked
                })
              }
            />
          </FormItem>

          {dashboard.data.enableAutoSave && (
            <FormItem title={t1.autoSaveInterval}>
              <EditorNumberItem
                size='lg'
                min={10}
                max={3600}
                step={30}
                value={dashboard.data.autoSaveInterval}
                notNull
                defaultZero={false}
                onChange={(v) =>
                  onChange((draft: Dashboard) => {
                    draft.data.autoSaveInterval = v
                  })
                }
              />
            </FormItem>
