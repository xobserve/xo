...
const PanelSettings = memo(({ panel, onChange }: PanelEditorProps) => {
  ...
  const externalPlugins = Object.keys(externalPanelPlugins).filter(
    (panelType) => !disabledPanels?.includes(panelType),
  )
  const builtinPlugins = Object.keys(builtinPanelPlugins).filter(
    (panelType) => !disabledPanels?.includes(panelType),
  )
  const isExternalPanel = !isEmpty(
    externalPlugins.find((panelType) => panelType == panel.type),
  )
    ...
    <>
      ...
      >
        ...
        <PanelEditItem title={t.transform} desc={t1.enableTransform}>
          <Switch
            isChecked={panel.enableTransform}
            onChange={(e) =>
              onChange((tempPanel: Panel) => {
                tempPanel.enableTransform = e.currentTarget.checked
              })
            }
          />
        </PanelEditItem>
        <PanelEditItem title={t1.conditionRender} desc={t1.conditionRenderTips}>
          <Switch
            isChecked={panel.enableConditionRender}
            onChange={(e) =>
              onChange((tempPanel: Panel) => {
                tempPanel.enableConditionRender = e.currentTarget.checked
              })
            }
          />
        </PanelEditItem>
            ...
            <PanelEditItem
              ...
              desc={
                panel.conditionRender.type == 'variable'
                  ? t1.conditionTips
                  : 'Check condition in a function, return true or false'
              }
            >
              {panel.conditionRender.type == 'variable' ? (
                <EditorInputItem
                  value={panel.conditionRender.value}
                  placeholder='e.g varNameA=value1, value1 can be regex'
                  onChange={(v) =>
