...
import {
  EditorInputItem,
  EditorNumberItem,
} from '../../../components/editor/EditorItem'
  ...
  return (
    <>
        ...
        <PanelAccordion title={t.basic} defaultOpen>
          <PanelEditItem title={t.palette} desc={t1.paletteTips}>
            <VStack spacing={1} alignItems='left'>
              {Object.keys(paletteMap).map((name) => (
                <HStack
                  width='fit-content'
                  spacing={0}
                  borderRadius={4}
                ...
                >
                  {paletteMap[name].map((color) => (
                    <Box
                      width='15px'
                      height='15px'
                      borderRadius='50%'
                      style={{ backgroundColor: paletteColorNameToHex(color) }}
                    />
                  ))}
                </HStack>
              ))}
          ...
          <PanelEditItem
            title='width'
            desc='panel width in dashboard, value range is [1,24], 24 is the same as 100%'
          >
            <EditorNumberItem
              min={1}
              max={24}
          ...
          <PanelEditItem title='Title position'>
            <RadionButtons
              ...
              options={['left', 'center'].map((p) => ({
                label: upperFirst(p),
                value: p,
              }))}
              onChange={(v) =>
                onChange((panel) => {
                  panel.styles.title.position = v
                })
              }
            />
          </PanelEditItem>
      ...
      <PanelAccordion title={t1.panelBorder} defaultOpen>
        ...
        {panel.styles.border == PanelBorderType.None && (
          <PanelEditItem title={t1.borderOnHover}>
            <Switch
              defaultChecked={panel.styles.borderOnHover}
              onChange={(e) => {
        ...
        <PanelEditItem title='Width reduction' desc='reduce panel width'>
          <EditorNumberItem
            ...
            max={1000}
            step={1}
            value={panel.styles.widthReduction}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='Height reduction' desc='reduce panel height'>
          <EditorNumberItem
            ...
            max={1000}
            step={1}
            value={panel.styles.heightReduction}
            onChange={(v) =>
              onChange((panel) => {
      ...
      <PanelAccordion title={t1.titleDecoration}>
        <PanelEditItem title='type'>
          <Select
            size='sm'
            value={panel.styles?.title?.decoration.type}
            onChange={(e) => {
          ...
          >
            {Object.keys(PanelTitleDecorationType).map((key) => (
              <option value={PanelTitleDecorationType[key]}>{key}</option>
            ))}
          </Select>
        </PanelEditItem>
        <PanelEditItem title='width'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.decoration.width}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='height'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.decoration.height}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='margin'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.decoration.margin}
            onChange={(v) =>
              onChange((panel) => {
      ...
      <PanelAccordion title={t1.titleStyles}>
        <PanelEditItem title='font size'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.fontSize}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='font weight'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.fontWeight}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='color'>
          <ColorPicker
            color={panel.styles.title.color}
            onChange={(c) =>
              onChange((panel: Panel) => {
        ...
        <PanelEditItem title='padding top'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.paddingTop}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='padding right'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.paddingRight}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='padding bottom'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.paddingBottom}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='padding left'>
          <EditorInputItem
            type='input'
            value={panel.styles.title?.paddingLeft}
            onChange={(v) =>
              onChange((panel) => {
      ...
      <PanelAccordion title={t1.panelDecoration}>
        <PanelEditItem title='type'>
          <DecorationSelect
            value={panel.styles?.decoration.type}
            onChange={(v) => {
              onChange((panel) => {
        ...
        <PanelEditItem title='reverse' desc={t1.reverseTips}>
          <Switch
            defaultChecked={panel.styles.decoration.reverse}
            onChange={(e) => {
              const checked = e.currentTarget.checked
              onChange((panel) => {
        ...
        <PanelEditItem title='width'>
          <EditorInputItem
            type='input'
            value={panel.styles.decoration.width}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='height'>
          <EditorInputItem
            type='input'
            value={panel.styles.decoration.height}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='top'>
          <EditorInputItem
            type='input'
            value={panel.styles.decoration.top}
            onChange={(v) =>
              onChange((panel) => {
        ...
        <PanelEditItem title='left'>
          <EditorInputItem
            type='input'
            value={panel.styles.decoration.left}
            onChange={(v) =>
              onChange((panel) => {
