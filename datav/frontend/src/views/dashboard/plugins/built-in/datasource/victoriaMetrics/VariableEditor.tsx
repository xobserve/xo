  ...
  return (
    <>
      <FormItem title={t1.useCurrentTime} alignItems='center'>
        ...
        {data.useCurrentTime && (
          <Text textStyle='annotation'>
            {dateTimeFormat(timeRange.start)} - {dateTimeFormat(timeRange.end)}
          </Text>
        )}
      </FormItem>
      <FormItem title={t1.queryType}>
        ...
        >
          {Object.keys(PromDsQueryTypes).map((k) => (
            <option value={PromDsQueryTypes[k]}>{PromDsQueryTypes[k]}</option>
          ))}
        </Select>
      </FormItem>
