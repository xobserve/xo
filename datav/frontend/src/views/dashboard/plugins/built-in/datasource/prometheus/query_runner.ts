...
export const runPrometheusQuery = async (
  panel: Panel,
  q: PanelQuery,
  range: TimeRange,
  ds: Datasource,
...
export const testPrometheusDatasource = async (ds: Datasource) => {
  ...
  try {
    ...
    const res = await requestApi.get(
      `/common/proxy/1?proxy_url=${ds.url}/api/v1/labels?match[]=up`,
    )
    if (res.status) {
      return true
    }
...
export const queryPromethuesVariableValues = async (variable: Variable) => {
  ...
  if (data.type == PromDsQueryTypes.LabelValues) {
    ...
    if (!isEmpty(data.regex)) {
      const res = await queryPrometheusAllMetrics(
        variable.datasource,
        data.useCurrentTime,
      )
      if (res.error) {
...
export const queryPrometheusAllMetrics = async (
  dsId,
...
export const queryPrometheusLabels = async (
  dsId,
  metric = '',
  useCurrentTimerange = true,
) => {
...
export const query_prometheus_alerts = async (
  panel: Panel,
  timeRange: TimeRange,
  ds: Datasource,
