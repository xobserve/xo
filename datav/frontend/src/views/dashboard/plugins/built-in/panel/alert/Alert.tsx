...
const AlertPanel = memo((props: AlertPanelProps) => {
  ...
  useBus(ResetPanelToolbalEvent + panel.id, () => {
    onViewOptionsChange(initViewOptions() as any)
  })

  useBus(ResetPanelToolbalViewModeEvent + panel.id, () => {
  ...
  const onActiveLabel = useCallback(
    (id) => {
  ...
  const [filterData, chartData]: [AlertRule[], any] = useMemo(() => {
    const stateFilter = options.filter.enableFilter
      ? !isEmpty(viewOptions.stateFilter)
        ? viewOptions.stateFilter
        : options.filter.state
      : null

    const ruleNameFilter = options.filter.enableFilter
      ? !isEmpty(viewOptions.ruleNameFilter)
        ? viewOptions.ruleNameFilter
        : options.filter.ruleName
      : ''
    const ruleLabelFilter = options.filter.enableFilter
      ? !isEmpty(viewOptions.ruleLabelsFilter)
        ? viewOptions.ruleLabelsFilter
        : options.filter.ruleLabel
      : ''
    const alertLabelFilter = options.filter.enableFilter
      ? !isEmpty(viewOptions.labelNameFilter)
        ? viewOptions.labelNameFilter
        : options.filter.alertLabel
      : ''

    const [result, chartData] = filterAlerts(
      props.data,
      stateFilter,
      ruleNameFilter,
      ruleLabelFilter,
        ...
        <Flex position='relative'>
          ...
          <CustomScrollbar>
            <Box
              ...
              width={
                width > 400
                  ? props.width - (toolbarOpen ? options.toolbar.width : 1)
                  : width
              }
              transition='all 0.3s'
            >
          ...
          {
            <Box
              className={toolbarOpen ? 'bordered-left' : null}
              height={props.height}
              maxHeight={props.height}
              width={toolbarOpen ? options.toolbar.width : 0}
              transition='all 0.3s'
              py='2'
            >
              <CustomScrollbar>
                {toolbarOpen && (
                  <AlertToolbar
                    ...
                    rulesCount={filterData.length}
                    alertsCount={alertsCount}
                    onViewLogChange={onViewOptionsChange}
                    viewOptions={viewOptions}
                    width={width}
                  />
                )}
      ...
      {viewMode == 'stat' && (
        <Box
          ...
          onClick={() =>
            onViewOptionsChange({
              ...viewOptions,
              viewMode: viewOptions.viewMode == 'list' ? 'stat' : 'list',
            })
          }
          pb='2'
        >
...
export const filterAlerts = (
  ...
  for (const r0 of data) {
    ...
    if (
      enableRuleStateFilter &&
      stateFilter !== null &&
      !stateFilter.includes(r0.state)
    ) {
      continue
    }
    // filter by rule name
    ...
    for (const alert of r0.alerts) {
      ...
      if (labelFilter) {
        ...
        for (const k in labelFilter) {
          if (
            !alert.labels[k]?.toLowerCase().match(labelFilter[k]?.toLowerCase())
          ) {
            matches = false
            break
          }
