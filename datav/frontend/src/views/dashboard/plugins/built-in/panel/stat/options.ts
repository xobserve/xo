...
export const parseOptions = (
  ...
  const cm =
    ...
    options.styles.colorMode
  const graphType =
    findRuleInOverride(override, StatRules.SeriesType) ?? options.styles?.style
  const fillOpacity =
    findRuleInOverride(override, StatRules.SeriesFill) ??
    options.styles.fillOpacity
  switch (cm) {
    case 'bg-solid':
    case 'bg-gradient':
  ...
  return {
    width: config.width,
    height:
      (config.height * config.panel.plugins.stat.styles.graphHeight) / 100,
    series: series,
    hooks: {},
    plugins: [],
    legend: {
