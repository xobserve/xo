...
const LogItem = memo((props: LogItemProps) => {
  ...
  const LabelLayout =
    options.labels.layout == LayoutOrientation.Horizontal ? HStack : Box
  ...
  switch (options.timeStampPrecision) {
    case 'ns':
      timeString =
        dateTimeFormat(timestamp, { format: 'YY-MM-DD HH:mm:ss.SSS' }) + nsPart
      break
    case 'us':
      timeString =
        dateTimeFormat(timestamp, { format: 'YY-MM-DD HH:mm:ss.SSS' }) + usPart
      break
    case 'ms':
      timeString = dateTimeFormat(timestamp, {
  ...
  const labelNameColor = (id) => {
    return options.styles.labelColorSyncChart
      ? getLabelNameColor(id, colorMode, colorGenerator)
      : options.styles.labelColor
  }
  ...
  return (
    <>
      <Flex
        ...
        pl='2'
        pr='4'
        onClick={options.enableDetails ? () => setCollapsed(!collapsed) : null}
        cursor={options.enableDetails ? 'pointer' : null}
        fontSize={options.styles.fontSize}
      >
        <HStack spacing={1}>
          {options.enableDetails && (
            <CollapseIcon
              collapsed={collapsed}
              fontSize='0.6rem'
              opacity='0.6'
              mt={options.showTime ? 0 : '6px'}
            />
          )}
          {options.showTime && (
            <Text
              minWidth={options.timeColumnWidth ?? 155}
              color={paletteColorNameToHex(timestampColor)}
            >
              {timeString}
            </Text>
          )}
        ...
        {options.labels.display.length > 0 && (
          <HStack
            ...
            maxW='100%'
            spacing={
              options.labels.layout == LayoutOrientation.Horizontal ? 2 : 3
            }
          >
            {Object.keys(labels).map(
              (key) =>
                options.labels.display.includes(key) && (
                  <LabelLayout
                    alignItems='start'
                    key={key + labels[key]}
                    spacing={0}
                    width={
                      (labelWidthMap[key] ?? options.labels.width ?? 100) + 'px'
                    }
                  >
                    <LabelName
                      name={!isEmpty(labels[key]) && key}
                      color={
                        log.labelHighlight.indexOf(key.toLowerCase()) >= 0
                          ? paletteColorNameToHex(options.styles.highlightColor)
                          : labelNameColor(formatLabelId(key, labels[key]))
                      }
                    />
                    {options.labels.layout == LayoutOrientation.Horizontal &&
                      !isEmpty(labels[key]) && <Text>=</Text>}
                    <LabelValue
                      value={labels[key]}
                      color={
                        log.labelHighlight.indexOf(
                          ...
                          : paletteColorNameToHex(
                              getStringColorMapping(
                                labels[key],
                                options.styles.labelValueColor,
                              ),
                            )
                      }
                      maxLines={options.labels.maxValueLines}
                    />
                  </LabelLayout>
                ),
            )}
        ...
        <Box color={paletteColorNameToHex(options.styles.contentColor)}>
          <pre
            style={{
              wordBreak: options.styles.wordBreak,
              whiteSpace: options.styles.wrapLine ? 'pre-wrap' : null,
            }}
          >
            <TextHighlight
              text={log.content}
              color={options.styles.highlightColor}
        ...
        <Box p='4' fontSize={options.styles.fontSize}>
          <VStack alignItems='left' className='bordered' p='2' mt='1'>
            ...
            {Object.keys(labels).map(
              (key) =>
                !isEmpty(labels[key]) && (
                  <HStack key={key + labels[key]} px='2' spacing={1}>
                    <Box minWidth={isMobileScreen ? '10em' : '20em'}>
                      <LabelName
                        name={key}
                        color={labelNameColor(formatLabelId(key, labels[key]))}
                      />
                    </Box>
                    <LabelValue
                      value={labels[key]}
          ...
          <Box p='2'>
            <pre>{toPrettyJSON(log.content)}</pre>
          </Box>
        </Box>
      )}
    </>
...
const LabelName = ({ name, color }: { name: string; color: string }) => {
  return (
    <Text color={paletteColorNameToHex(color)} minW='fit-content'>
      {name}
    </Text>
  )
}
...
const LabelValue = ({
  ...
  return (
    <Text
      color={paletteColorNameToHex(color)}
      wordBreak='break-all'
      noOfLines={maxLines}
    >
      {value}
