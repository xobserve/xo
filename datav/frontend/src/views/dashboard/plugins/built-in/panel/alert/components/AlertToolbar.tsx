  ...
  return (
    <Box>
      ...
      <VStack alignItems='left' fontSize='0.8rem' mt='2' px='2' spacing={2}>
        <Box>
          ...
          <RadionButtons
            ...
            onChange={(v) => onViewLogChange({ ...viewOptions, viewMode: v })}
          />
        </Box>
        <Box>
          ...
          <RadionButtons
            ...
            value={viewOptions.barType}
            onChange={(v) => onViewLogChange({ ...viewOptions, barType: v })}
          />
        </Box>
        <Box>
          <Text mb='2'>Rule name filter</Text>
          <EditorInputItem
            value={viewOptions.ruleNameFilter}
            onChange={(v) =>
              onViewLogChange({ ...viewOptions, ruleNameFilter: v })
            }
            placeholder='support multi regex'
          />
        </Box>
        <Box>
          <Text mb='2'>Rule label filter</Text>
          <EditorInputItem
            value={viewOptions.ruleLabelsFilter}
            onChange={(v) =>
              onViewLogChange({ ...viewOptions, ruleLabelsFilter: v })
            }
        ...
        <Box>
          ...
          <EditorInputItem
            value={viewOptions.labelNameFilter}
            onChange={(v) =>
              onViewLogChange({ ...viewOptions, labelNameFilter: v })
            }
            placeholder={`{service="api-gateway", instance=~"cluster-cn-.+"}`}
          />
        </Box>
        <Box>
          ...
          <MultiRadionButtons
            ...
            value={viewOptions.stateFilter}
            onChange={(v) =>
              onViewLogChange({ ...viewOptions, stateFilter: v })
            }
          />
        </Box>
      </VStack>
      ...
      <HStack mt='4' px='2'>
        ...
        <Checkbox
          isChecked={viewOptions.persist}
          onChange={(e) =>
            onViewLogChange({
              ...viewOptions,
              persist: e.currentTarget.checked,
            })
          }
        />
      </HStack>
