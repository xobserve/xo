...
const ContextMenu = memo(({ props, options, data, container }: Props) => {
        ...
        over.addEventListener('mouseup', (e) => {
          ...
          if (e.clientX == clientX && e.clientY == clientY) {
            if (seriesIdx.current != null && dataIdx.current != null) {
              setCoords((cd) =>
                cd ? null : { x: coordX.current, y: coordY.current },
              )
  ...
  const startTime = roundDsTime(
    plotInstance.current?.posToVal(xVal.current, 'x'),
  )
            ...
            >
              ...
              <HStack mt='1'>
                <Box
                  width='10px'
                  height='4px'
                  background={seriesIdx.current.color}
                  mt='2px'
                ></Box>
                <Text maxW={isMobileScreen ? props.width / 2 : null}>
                  {seriesIdx.current.name}
                </Text>
              </HStack>
              <Divider mt='2' />
              <VStack
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (isFunction(onGraphClick)) {
                            const tData = commonInteractionEvent(onGraphClick, {
                              series: seriesIdx.current,
                              time: startTime,
                            })
                            return tData
                          } else {
