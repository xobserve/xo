        ...
        <>
          <PanelAccordion title='Tooltip'>
            <PanelEditItem title={t.mode}>
              <RadionButtons
                ...
                value={panel.plugins.graph.tooltip.mode}
                onChange={(v) =>
          ...
          <PanelAccordion title='Legend'>
            <PanelEditItem title={t.mode}>
              <RadionButtons
                ...
                value={panel.plugins.graph.legend.mode}
                onChange={(v) =>
                  onChange((panel: GraphPanel) => {
              ...
              <>
                <PanelEditItem title={t1.placement}>
                  <RadionButtons
                    ...
                    value={panel.plugins.graph.legend.placement}
                    onChange={(v) =>
                      onChange((panel: GraphPanel) => {
                ...
                {panel.plugins.graph.legend.placement == 'right' && (
                  <PanelEditItem title={t1.width}>
                    <EditorNumberItem
                      value={panel.plugins.graph.legend.width}
                      min={100}
                      step={50}
                      onChange={(v) =>
                        onChange((panel: GraphPanel) => {
                          panel.plugins.graph.legend.width = v <= 100 ? 100 : v
                        })
                      }
                    />
                  </PanelEditItem>
                ...
                {panel.plugins.graph.legend.placement == 'right' && (
                  <PanelEditItem title={t1.nameWidth} desc={t1.nameWidthTips}>
                    <EditorInputItem
                      value={panel.plugins.graph.legend.nameWidth}
                      onChange={(v) =>
                        onChange((panel: GraphPanel) => {
                ...
                <PanelEditItem title={t1.values} desc={t1.valuesTips}>
                  <PopoverSelect
                    value={panel.plugins.graph.legend.valueCalcs}
                    isMulti
                    options={Object.keys(ValueCalculationType).map((k) => ({
                      label: k,
                      value: k,
                    }))}
                    onChange={(v) =>
                ...
                <PanelEditItem title={t1.showValuesName}>
                  <Switch
                    defaultChecked={panel.plugins.graph.legend.showValuesName}
                    onChange={(e) =>
                      onChange((panel: GraphPanel) => {
                        panel.plugins.graph.legend.showValuesName =
          ...
          <PanelAccordion title={t1.graphStyles}>
            <PanelEditItem title={t.type}>
              <RadionButtons
                ...
                value={panel.plugins.graph.styles.style}
                onChange={(v) =>
                  onChange((panel: GraphPanel) => {
            ...
            {panel.plugins.graph.styles.style == 'bars' && (
              <>
                <PanelEditItem title={t1.barRadius}>
                  <EditorNumberItem
                    value={panel.plugins.graph.styles.barRadius}
                    min={0}
                    max={0.5}
                    step={0.1}
                    onChange={(v) =>
                ...
                <PanelEditItem title={t1.barGap}>
                  <EditorNumberItem
                    value={panel.plugins.graph.styles.barGap}
                    min={0}
                    max={100}
                    step={5}
                    onChange={(v) =>
            ...
            )}
            {
              <>
                <PanelEditItem title={t1.lineWidth}>
                  <EditorSliderItem
                    value={panel.plugins.graph.styles.lineWidth}
                    min={0}
                    max={10}
                    step={1}
                    onChange={(v) =>
                ...
                <PanelEditItem title={t1.gradient}>
                  <RadionButtons
                    ...
                    value={panel.plugins.graph.styles.gradientMode}
                    onChange={(v) =>
                      onChange((panel: GraphPanel) => {
                ...
                <PanelEditItem title={t1.opacity}>
                  <EditorSliderItem
                    value={panel.plugins.graph.styles.fillOpacity}
                    min={0}
                    max={100}
                    step={1}
                    onChange={(v) => {
                ...
                <PanelEditItem title={t1.showPoints}>
                  <RadionButtons
                    ...
                    value={panel.plugins.graph.styles.showPoints}
                    onChange={(v) =>
                      onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title={t1.pointSize}>
              <EditorSliderItem
                value={panel.plugins.graph.styles.pointSize}
                min={1}
                max={20}
                step={1}
                onChange={(v) =>
            ...
            <PanelEditItem title={t1.connectNull}>
              <Switch
                defaultChecked={panel.plugins.graph.styles.connectNulls}
                onChange={(e) =>
                  onChange((panel: GraphPanel) => {
                    panel.plugins.graph.styles.connectNulls =
                      e.currentTarget.checked
                  })
                }
              />
            ...
            <PanelEditItem title='Stack'>
              <Switch
                defaultChecked={panel.plugins.graph.styles.enableStack}
                onChange={(e) =>
                  onChange((panel: GraphPanel) => {
                    panel.plugins.graph.styles.enableStack =
                      e.currentTarget.checked
                  })
                }
              />
          ...
          <PanelAccordion title={t.axis}>
            <PanelEditItem title={t.label}>
              <EditorInputItem
                value={panel.plugins.graph.axis.label}
                onChange={(v) =>
                  onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title={t1.showGrid}>
              <RadionButtons
                ...
                value={panel.plugins.graph.axis.showGrid}
                onChange={(v) =>
                  onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title='Show X axis'>
              <Switch
                defaultChecked={panel.plugins.graph.axis.showX}
                onChange={(e) =>
                  onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title='Show Y axis'>
              <Switch
                defaultChecked={panel.plugins.graph.axis.showY}
                onChange={(e) =>
                  onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title={t.scale}>
              <RadionButtons
                ...
                value={panel.plugins.graph.axis.scale}
                onChange={(v) =>
                  onChange((panel: GraphPanel) => {
            ...
            {panel.plugins.graph.axis.scale == 'log' && (
              <PanelEditItem title='Scale Base'>
                <RadionButtons
                  options={[
                    { label: 'Base 2', value: '2' },
                    { label: 'Base 10', value: '10' },
                  ]}
                  value={panel.plugins.graph.axis.scaleBase as any}
                  onChange={(v) =>
                    onChange((panel: GraphPanel) => {
          ...
          <PanelAccordion title='Value'>
            <PanelEditItem title={t.unit}>
              <UnitPicker
                value={panel.plugins.graph.value}
                onChange={
                  (v: Units) =>
                    onChange((panel: GraphPanel) => {
            ...
            <PanelEditItem title={t.decimal}>
              <EditorNumberItem
                value={panel.plugins.graph.value.decimal}
                min={0}
                max={5}
                step={1}
                onChange={(v) =>
          ...
          <PanelAccordion title={t1.alertCorrelation}>
            <PanelEditItem title='Enable' desc={t1.alertCorrelationTips}>
              <Switch
                defaultChecked={panel.plugins.graph.enableAlert}
                onChange={(e) =>
                  onChange((panel: GraphPanel) => {
          ...
          {panel.plugins.graph.enableAlert && (
            <AlertFilterEditor
              panel={panel as any}
              onChange={onChange}
              filter={panel.plugins.graph.alertFilter}
            />
          )}
        </>
      )}
