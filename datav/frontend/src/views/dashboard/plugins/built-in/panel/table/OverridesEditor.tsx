  ...
  switch (override.type) {
    ...
    case TableRules.ColumnWidth:
      return (
        <EditorInputItem
          ...
          onChange={onChange}
          size='sm'
          placeholder='css width, e.g 100px, 20%, auto'
        />
      )
    ...
    case TableRules.ColumnFilter:
      return (
        <RadionButtons
          size='sm'
          options={[
            { label: t1.seriesFilter1, value: 'number' },
            { label: t1.seriesFilter2, value: 'string' },
          ]}
          value={override.value}
          onChange={onChange}
        />
      ...
      return (
        <Box>
          <RadionButtons
            size='sm'
            options={[
              { label: t.descend, value: 'descend' },
              { label: t.ascend, value: 'ascend' },
            ]}
            value={override.value}
            onChange={(v) => {
              onChange(v)
    ...
    case TableRules.ColumnDisplay:
      return (
        <Checkbox
          size='lg'
          isChecked={isEmpty(override.value) ? true : override.value}
          onChange={(e) => onChange(e.currentTarget.checked)}
        />
      )
    case TableRules.ColumnTransform:
      return (
        <CodeEditorModal
          value={isEmpty(override.value) ? transformFunc : override.value}
          onChange={onChange}
        />
      )
