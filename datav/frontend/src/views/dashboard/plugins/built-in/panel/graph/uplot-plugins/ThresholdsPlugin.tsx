...
export const ThresholdsPlugin = ({
  ...
  const scaleKey = 'y'
  const dashSegments =
    display === ThresholdDisplay.DashedLine ||
    display === ThresholdDisplay.AreaDashedLine
      ? [10, 10]
      : undefined
  ...
  function addLines(u: uPlot, yScaleKey: string, steps: Threshold[]) {
    ...
    for (let idx = 1; idx < steps.length; idx++) {
      ...
      if (transparentIndex >= idx && idx > 0) {
        color = tinycolor(
          paletteColorNameToHex(steps[idx - 1].color, colorMode),
        )
      } else {
