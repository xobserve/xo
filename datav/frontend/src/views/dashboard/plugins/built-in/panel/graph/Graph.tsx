...
const GraphPanel = memo((props: GraphPanelProps) => {
  ...
  const onChartCreate = useCallback(
    (chart) => {
      setUplot(chart)
    ...
    <>
      {isEmpty(props.data) ? (
        ...
        <Box
          ...
          position='relative'
        >
          {!isEmpty(props?.panel.plugins.graph.axis?.label) && (
            <Text
              fontSize='sm'
              position='absolute'
              ml='3'
            ...
            >
              {props.panel.plugins.graph.axis.label}
            </Text>
          )}
          {options && (
            <GraphLayout
              ...
              height={props.height}
              legend={
                props.panel.plugins.graph.legend.mode == 'hidden' ? null : (
                  <LegendTable
                    panel={props.panel}
            ...
            >
              {(vizWidth: number, vizHeight: number) => {
                ...
                const thresholdsOverride = props.panel.overrides.find(
                  (override) =>
                    findRuleInOverride(override, GraphRules.SeriesThresholds),
                )
                const v = findRuleInOverride(
                  thresholdsOverride,
                  GraphRules.SeriesThresholds,
                )
