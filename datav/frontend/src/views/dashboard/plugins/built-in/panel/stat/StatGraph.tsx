...
const StatGraph = memo((props: Props) => {
  ...
  const [valueText, options, legend, color, valueColor, legendColor, bgColor] =
    useMemo(() => {
      ...
      const calcOverride = findRuleInOverride(
        override,
        StatRules.SeriesValueCalc,
      )
      const calc = calcOverride ?? statOptions.value.calc
      const value = calcValueOnSeriesData(data, calc)
      const valueText = formatUnit(
        value,
        findRuleInOverride(override, StatRules.SeriesUnit)?.units ??
          statOptions.value.units,
        findRuleInOverride(override, StatRules.SeriesDecimal) ??
      ...
      const cm =
        findRuleInOverride(override, StatRules.SeriesColorMode) ??
        statOptions.styles.colorMode
      let valueColor
      let legendColor
      switch (cm) {
        case 'value':
      ...
      >
        <Box h='100%'>
          {statOptions.styles.layout == 'horizontal' && (
            <>
              {graphHeight < 100 && (
                <Box
                  height={
                    height > statOptions.styles.hideGraphHeight
                      ? `${100 - graphHeight}%`
                      : '100%'
                  }
                  className='stat-graph-text'
                >
                  <Center
                    height='100%'
                    pt={height > statOptions.styles.hideGraphHeight ? 2 : 0}
                  >
                    <Flex
                      width='100%'
                      px={4}
                      alignItems='center'
                      justifyContent={
                        statOptions.showLegend ? 'space-between' : 'center'
                      }
                    >
                      {statOptions.showLegend && (
                        <LegentText
                          legend={legend}
                          height={height}
                          width={width}
                      ...
                      <ValueText
                        ...
                        width={width}
                        height={height}
                        layout={statOptions.styles.layout}
                        color={valueColor}
                      />
                    </Flex>
                  </Center>
          ...
          {(statOptions.styles.layout == 'vertical' ||
            statOptions.styles.layout == 'auto') && (
            <>
              {graphHeight < 100 && (
                <TextContainer
                  height={
    ...
    <>
      {options && (
        <UplotReact
          ...
          onCreate={() => {}}
        >
          {props.panel.plugins.stat.showTooltip && (
            <Tooltip
              props={props}
              options={options}
              data={[data]}
