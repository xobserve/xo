...
const LogChart = memo((props: Props) => {
  ...
  const options =
    panel.type == PanelTypeLog
      ? panel.plugins.log.chart
      : panel.plugins.alert.chart
      ...
      for (const log of props.data) {
        ...
        if (typeof log.labels === 'string') {
          ...
          keys.forEach((k, i) => {
            ...
            if (
              panel.type == PanelTypeLog &&
              panel.plugins.log.chart.categorize != k
            ) {
              if (i < keys.length - 1) {
                return
              }
              ...
              if (!labeled) {
                labelId = formatLabelId(
                  panel.plugins.log.chart.categorize,
                  undefined,
                )
              }
            } else {
  ...
  const tooltipOptions =
    panel.type == PanelTypeAlert
      ? panel.plugins.alert.chart.tooltip
      : panel.plugins.log.chart.tooltip
...
const getSeriesColor = (
  ...
  if (kv[0] == panel.plugins.log?.chart.categorize) {
    const color = paletteColorNameToHex(
      getStringColorMapping(kv[1], panel.plugins.log.styles.labelValueColor),
    )
    return color != 'inherit'
      ? color
      : getLabelNameColor(name, colorMode, colorGenerator)
  }
