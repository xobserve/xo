...
const ErrorOkChart = memo((props: Props) => {
  ...
  const [timeline, names, data] = useMemo(() => {
    ...
    const timeline = timeBucks.map((t) =>
      dateTimeFormat(t * 1000, { format: timeFormat }),
    )
  ...
  return (
    <>
      <Text
        visibility={totalCount !== null ? 'visible' : 'hidden'}
        opacity={0.8}
        mb='2'
        fontSize='0.8rem'
        fontWeight={500}
      >
        {displayCount && displayCount + ' Results / '} {totalCount} Hits{' '}
      </Text>
      <ChartComponent
        key={colorMode}
    ...
    <PanelAccordion title={'Chart'}>
      <PanelEditItem title={lang == 'en' ? 'Type' : '图表类型'}>
        <RadionButtons
          ...
          value={panel.plugins[panelType].chart.type}
          onChange={(v) =>
      ...
      <PanelEditItem title={lang == 'en' ? 'Height' : '图表高度'}>
        <EditorNumberItem
          value={panel.plugins[panelType].chart.height}
          onChange={(v) =>
            onChange((panel: Panel) => {
      ...
      <PanelEditItem title={'Stack'}>
        <Switch
          defaultChecked={panel.plugins[panelType].chart.stack}
          onChange={(e) =>
            onChange((panel: Panel) => {
      ...
      <PanelEditItem title={'Top'}>
        <EditorNumberItem
          value={panel.plugins[panelType].chart.top}
          onChange={(v) =>
            onChange((panel: Panel) => {
      ...
      <PanelEditItem title={'Right'}>
        <EditorNumberItem
          value={panel.plugins[panelType].chart.right}
          onChange={(v) =>
            onChange((panel: Panel) => {
      ...
      <PanelEditItem title={'Bottom'}>
        <EditorNumberItem
          value={panel.plugins[panelType].chart.bottom}
          onChange={(v) =>
            onChange((panel: Panel) => {
      ...
      <PanelEditItem title={'Left'}>
        <EditorNumberItem
          value={panel.plugins[panelType].chart.left}
          onChange={(v) =>
            onChange((panel: Panel) => {
