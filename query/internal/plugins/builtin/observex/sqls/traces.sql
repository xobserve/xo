CREATE TABLE observex_traces.trace_index
(
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_trace_index ON CLUSTER cluster AS observex_traces.trace_index
ENGINE = Distributed("cluster", "observex_traces", trace_index, cityHash64(traceId));


CREATE TABLE observex_traces.trace_spans
(
    `startTime` UInt64 CODEC(DoubleDelta, LZ4),
    `traceId` FixedString(32) CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_trace_spans ON CLUSTER cluster AS observex_traces.trace_spans
ENGINE = Distributed("cluster", "observex_traces", trace_spans, cityHash64(traceId));





CREATE TABLE observex_traces.top_level_operations
(   
    tenant LowCardinality(String) CODEC(ZSTD(1)),
    namespace LowCardinality(String) CODEC(ZSTD(1)),
    group LowCardinality(String) CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_top_level_operations ON CLUSTER cluster AS observex_traces.top_level_operations
ENGINE = Distributed("cluster", "observex_traces", top_level_operations, cityHash64(rand()));

CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.sub_root_operations_mv ON CLUSTER cluster
TO observex_traces.top_level_operations
AS SELECT DISTINCT
    tenant, 
    namespace, 
    group,
    serviceName,
    name
FROM observex_traces.trace_index AS A, observex_traces.trace_index AS B
WHERE (A.serviceName != B.serviceName) AND (A.parentId = B.spanId);

CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.root_operations_mv ON CLUSTER cluster
TO observex_traces.top_level_operations
AS SELECT DISTINCT
    tenant, 
    namespace, 
    ...
    serviceName,
    name
FROM observex_traces.trace_index
WHERE parentId = '';

CREATE TABLE observex_traces.service_operations
(
    tenant LowCardinality(String) CODEC(ZSTD(1)),
    namespace LowCardinality(String) CODEC(ZSTD(1)),
    group LowCardinality(String) CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.service_operations_mv ON CLUSTER cluster
TO observex_traces.service_operations
AS SELECT DISTINCT
    tenant, 
    namespace, 
    group,
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_service_operations ON CLUSTER cluster AS observex_traces.service_operations
ENGINE = Distributed("cluster", "observex_traces", service_operations, cityHash64(rand()));


CREATE TABLE observex_traces.usage_explorer
(
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_usage_explorer ON CLUSTER cluster AS observex_traces.usage_explorer
ENGINE = Distributed("cluster", "observex_traces", usage_explorer, cityHash64(rand()));

CREATE MATERIALIZED VIEW IF NOT EXISTS observex_traces.usage_explorer_mv ON CLUSTER cluster
TO observex_traces.usage_explorer
AS SELECT
...
CREATE TABLE observex_traces.trace_error_index
(
    `timestamp` DateTime64(9) CODEC(DoubleDelta, LZ4),
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_trace_error_index ON CLUSTER cluster AS observex_traces.trace_error_index
ENGINE = Distributed("cluster", "observex_traces", trace_error_index, cityHash64(groupID));


CREATE TABLE observex_traces.span_attributes_keys
(
    tenant LowCardinality(String) CODEC(ZSTD(1)),
    namespace LowCardinality(String) CODEC(ZSTD(1)),
    group LowCardinality(String) CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_span_attributes_keys ON CLUSTER cluster AS observex_traces.span_attributes_keys
ENGINE = Distributed("cluster", "observex_traces", span_attributes_keys, cityHash64(rand()));


CREATE TABLE observex_traces.span_attributes
(
    `startTime` UInt64 CODEC(DoubleDelta, LZ4),
    tenant LowCardinality(String) CODEC(ZSTD(1)),
    namespace LowCardinality(String) CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS observex_traces.distributed_span_attributes ON CLUSTER cluster AS observex_traces.span_attributes
ENGINE = Distributed("cluster", "observex_traces", span_attributes, cityHash64(rand()));